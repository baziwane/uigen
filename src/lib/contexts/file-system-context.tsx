...
export function FileSystemProvider({
  ...
  const handleToolCall = useCallback(
    (toolCall: ToolCall) => {
      const { toolName, args: argsParam, input } = toolCall;
      // Support both old 'args' format and new 'input' format from AI SDK v6
      const args = argsParam || input;

      // Handle str_replace_editor tool
      if (toolName === "str_replace_editor" && args) {
        const command = args.command as string;
        const path = args.path as string;
        const file_text = args.file_text as string | undefined;
        const old_str = args.old_str as string | undefined;
        const new_str = args.new_str as string | undefined;
        const insert_line = args.insert_line as number | undefined;
