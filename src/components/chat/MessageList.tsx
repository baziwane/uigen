...
// Helper to extract text content from message parts for use as a fallback key
function getMessageTextContent(message: UIMessage): string {
  const textPart = message.parts?.find((p) => p.type === "text");
  return textPart && "text" in textPart ? textPart.text : "";
}

// Check if a part is a tool invocation part (handles both old and new formats)
function isToolPart(part: UIMessage["parts"][number]): part is UIMessage["parts"][number] & { toolCallId: string; toolName?: string; input?: unknown; output?: unknown; state?: string } {
    ...
    <div className="flex flex-col h-full overflow-y-auto px-4 py-6">
      <div className="space-y-6 max-w-4xl mx-auto w-full">
        {messages.map((message) => (
          <div
            key={message.id || getMessageTextContent(message)}
            className={cn(
              "flex gap-4",
                  ...
                  {message.parts && message.parts.length > 0 ? (
                    <>
                      {message.parts.map((part, partIndex) => {
                        ...
                        if (isToolPart(part)) {
                          // Handle tool invocations - extract tool name from type or toolName property
                          ...
                          return (
                            <ToolInvocationDisplay
                              ...
                              toolInvocation={{
                                ...
                                state: "state" in part ? (part.state === "output-available" ? "result" : "call") : "call",
                        ...
                        if (part.type === "source-url" || part.type === "source-document") {
                          return (
                            <div key={partIndex} className="mt-2 text-xs text-neutral-500">
                              Source: {"url" in part ? part.url : part.sourceId}
                            </div>
                          );
                        }
